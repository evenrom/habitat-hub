...
(async () => {
  ...
  try {
    ...
    const headingFont = await page.evaluate(() => {
        ...
        if (!h2) return null;
        return window.getComputedStyle(h2).fontFamily;
    });
    console.log('Heading Font:', headingFont);
    if (headingFont && headingFont.includes('Noto Serif')) throw new Error('Heading still uses Noto Serif');

    // Test Pointer Events and Cursor on Render Node
    ...
    // Dispatch click
    await page.evaluate(() => document.getElementById('test_node').dispatchEvent(new Event('click')));
    ...
    console.log('Modal visible and correct image src:', isModalVisibleAndHasCorrectSrc);
    if (!isModalVisibleAndHasCorrectSrc) throw new Error('Modal did not open or src is incorrect.');
